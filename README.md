...
3. The old course pages redirect to GitHub Pages; the comments API remains public. Instructor moderation is retained at `https://phs651-spring-2019.lmaowisc.chatgpt.site/?moderate=1#comments` for the already authenticated instructor account; other visitors are redirected. Replace the existing platform-specific moderator authentication before independently deploying `server.js`. It expects trusted hosting-platform authentication headers, which must not be trusted from arbitrary internet clients. The local preview strips them and does not provide instructor moderation.
